...
import type { ProduceFilters, CreateProduceInput, UpdateProduceInput, PartialUpdateProduceInput } from "@/types/produce";
...
export function useUpdateProduce() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: UpdateProduceInput }) => produceApi.updateProduce(id, data),
...
export function usePartialUpdateProduce() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: ({ id, data }: { id: string; data: PartialUpdateProduceInput }) => produceApi.partialUpdateProduce(id, data),
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ["produces"] });
      queryClient.invalidateQueries({ queryKey: ["produce", variables.id] });
    },
