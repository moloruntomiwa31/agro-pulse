...
import type { Order, OrderFilters, OrderListResponse, CreateOrderInput, UpdateOrderStatusInput } from "@/types/order";
...
export async function partialUpdateOrder(id: string, data: Partial<Order>): Promise<Order> {
  return apiRequest<Order>(`/api/orders/${id}/`, {
    method: "PATCH",
    body: JSON.stringify(data),
  });
...
export async function getOrdersByStatus(status: string): Promise<OrderListResponse> {
