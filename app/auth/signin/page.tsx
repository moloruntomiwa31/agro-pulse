  ...
  return (
    <div>
      ...
      <div className="mb-7 flex items-center gap-4">
        ...
        <span className="text-xs font-medium text-stone-600">or continue with email</span>
        <div className="flex-1 h-px bg-stone-200" />
      </div>
      ...
      <form className="flex flex-col gap-4.5">
        <div>
          ...
          <input id="signin-email" type="email" placeholder="you@example.com" required className="auth-input" />
        </div>
        <div>
          <label htmlFor="signin-password" className="auth-label">Password</label>
          <input id="signin-password" type="password" placeholder="Enter your password" required className="auth-input" />
        </div>

        {/* Role selector */}
        <div>
          <label htmlFor="signin-role" className="auth-label">I am a</label>
          <div className="auth-role-row">
            {[
              { value: "farmer", emoji: "🌾", label: "Farmer" },
              { value: "buyer",  emoji: "🛒", label: "Buyer" },
              { value: "rider",  emoji: "🚴", label: "Rider" },
            ].map((r) => (
              <label key={r.value} className="auth-role-chip">
                <input type="radio" name="role" value={r.value} defaultChecked={r.value === "farmer"} className="hidden" />
        ...
        <div className="flex justify-end">
          <a href="#" className="text-xs font-medium text-forest-400 no-underline hover:text-forest-500">Forgot password?</a>
        </div>
      ...
      <p className="mt-7 text-center text-sm text-green-700">
        ...
        <Link href="/auth/signup" className="font-semibold text-forest-400 no-underline hover:text-forest-500">
          Create account
        </Link>
      </p>
